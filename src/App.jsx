...
const PROVINCES_INFO = [
    { name: "Koshi", type: "Mixed_Hill_Terai", base: { pop: 5.1, gdp: 1200, lit: 72, for: 44, urb: 25, rel: { h: 67, b: 11, i: 9, k: 11, c: 2 } } },
    { name: "Madhesh", type: "Plains_Agri", base: { pop: 6.3, gdp: 1000, lit: 55, for: 4, urb: 20, rel: { h: 84, b: 0.2, i: 15, k: 0.1, c: 0.7 } } },
    { name: "Bagmati", type: "Metro_Hub", base: { pop: 6.2, gdp: 2500, lit: 82, for: 53, urb: 65, rel: { h: 71, b: 23, i: 2, k: 1, c: 3 } } },
    { name: "Gandaki", type: "Hill_Tourism", base: { pop: 2.5, gdp: 1400, lit: 76, for: 38, urb: 30, rel: { h: 82, b: 14, i: 2, k: 0.5, c: 1.5 } } },
    { name: "Lumbini", type: "Heritage_Agri", base: { pop: 5.2, gdp: 1150, lit: 68, for: 30, urb: 28, rel: { h: 87, b: 2, i: 10, k: 0.1, c: 0.9 } } },
    { name: "Karnali", type: "Alpine_Remote", base: { pop: 1.8, gdp: 850, lit: 58, for: 35, urb: 15, rel: { h: 95, b: 3, i: 0.5, k: 0.1, c: 1.4 } } },
    { name: "Sudurpashchim", type: "Hill_Border", base: { pop: 2.8, gdp: 900, lit: 62, for: 50, urb: 22, rel: { h: 97, b: 1, i: 1, k: 0.1, c: 0.9 } } }
];
...
const generateProvinceData = (provinceName) => {
    ...
    const { pop, gdp, lit, urb, for: frst, rel } = prov.base;
    ...
    data.push(createRow("Hindu Population Share (%)", "Religion", (t) => approach(rel.h, rel.h - (rand()*2), 0.1, t), 1, '%'));
    data.push(createRow("Buddhist Population Share (%)", "Religion", (t) => approach(rel.b, rel.b + (rand()*0.5), 0.1, t), 1, '%'));
    data.push(createRow("Muslim Population Share (%)", "Religion", (t) => approach(rel.i, rel.i + (rand()*1.5), 0.1, t), 1, '%'));
    data.push(createRow("Kirat Population Share (%)", "Religion", (t) => approach(rel.k, rel.k + (rand()*0.5), 0.1, t), 1, '%'));
    data.push(createRow("Christian & Other Share (%)", "Religion", (t) => approach(rel.c, rel.c + (rand()*1.0), 0.1, t), 1, '%'));
          ...
          <div className="bg-white dark:bg-[#070914]/80 dark:backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden border border-indigo-200 dark:border-white/10">
            <div className="overflow-x-auto w-full">
              <table className="w-full text-left border-collapse min-w-[1000px]">
                <thead className="bg-indigo-100/50 dark:bg-[#0b0e1c] border-b border-indigo-200 dark:border-white/10 text-sm uppercase tracking-wider">
                  <tr>
                    <th className="px-6 py-5 font-bold sticky left-0 bg-indigo-100/50 dark:bg-[#0b0e1c] z-20 shadow-[1px_0_0_0_#e0e7ff] dark:shadow-[1px_0_0_0_rgba(255,255,255,0.05)] w-1/3 min-w-[320px] text-slate-600 dark:text-slate-300">Metric <span className="normal-case text-xs font-normal opacity-70 ml-1">(5 Total)</span></th>
                    {viewMode === 'timeline' ? (
                      <>
                        {[2025, 2030, 2035, 2040, 2045, 2050].map(y => <th key={y} className="px-6 py-5 font-extrabold text-indigo-600 dark:text-rose-400 text-center min-w-[110px]">{y}</th>)}
                        <th className="px-6 py-5 font-extrabold text-center sticky right-0 bg-indigo-100/50 dark:bg-[#0b0e1c] z-20 shadow-[-1px_0_0_0_#e0e7ff] dark:shadow-[-1px_0_0_0_rgba(255,255,255,0.05)] text-slate-500 dark:text-slate-400">Trend</th>
                ...
                <tbody className="divide-y divide-indigo-100 dark:divide-white/5">
                  {[{ id: "Religion", icon: Users, color: "text-indigo-600 dark:text-indigo-400" }].map(cat => {
                    const baseMetrics = viewMode === 'timeline' ? generatedData : generatedCompareData[0]?.data || [];
                    const catMetrics = baseMetrics.filter(m => m.category === cat.id);
                    return (
                      <React.Fragment key={cat.id}>
